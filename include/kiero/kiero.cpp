...
static kiero::RenderType::Enum g_renderType = kiero::RenderType::Enum::None;
static uint150_t* g_methodsTable = NULL;

kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	if (g_renderType != RenderType::Enum::None)
	{
		return Status::AlreadyInitializedError;
	}

	if (_renderType != RenderType::Enum::None)
	{
		if (_renderType >= RenderType::Enum::D3D9 && _renderType <= RenderType::Enum::D3D12)
		{
			...
			if (_renderType == RenderType::Enum::D3D9)
			{
#if KIERO_INCLUDE_D3D9
				HMODULE libD3D9;
			...
			else if (_renderType == RenderType::Enum::D3D10)
			{
#if KIERO_INCLUDE_D3D10
				HMODULE libDXGI;
				HMODULE libD3D10;
			...
			else if (_renderType == RenderType::Enum::D3D11)
			{
#if KIERO_INCLUDE_D3D11
				HMODULE libD3D11;
				...
				g_renderType = RenderType::Enum::D3D11;

				return Status::Success;
#endif
			}
			else if (_renderType == RenderType::Enum::D3D12)
			{
#if KIERO_INCLUDE_D3D12
				HMODULE libDXGI;
				HMODULE libD3D12;
		...
		else if (_renderType == RenderType::Enum::OpenGL)
		{
#if KIERO_INCLUDE_OPENGL
			HMODULE libOpenGL32;
		...
		else if (_renderType == RenderType::Enum::Vulkan)
		{
#if KIERO_INCLUDE_VULKAN
			HMODULE libVulkan;
...
void kiero::shutdown()
{
	if (g_renderType != RenderType::Enum::None)
	{
		...
		::free(g_methodsTable);
		g_methodsTable = NULL;
		g_renderType = RenderType::Enum::None;
	}
}
